...
//vista in ORDINE DECRESCENTE
void inDEOrder(tree t) {
	if (!empty(t)) {
		inDEOrder(right(t));
		printf("\t");
		showEl(root(t));
...
//i parametri vanno SETTATI nelle primitive che fanno i confronti
...
tree BSTread(FILE *f)
{
	...
	while (1)
	{
		...
		//ctrl contiene il numero di elementi letti
		if (feof(f) || ctrl != 2)
			break;
...
//si ferma al valore di stop.numeroaccessi
tree CreaAlberoBST()
{
	tree t = emptyTree();
	sito e;
